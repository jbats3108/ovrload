...
Desktop keeps the all-blocks table with separate **Rest** and **Warm-up** columns. Routine profile and Deload live in a collapsed **Routine settings** strip under the title (same fields as the mobile Routine sheet). Profile-owned Target / Floor and shared Rest / Warm-up collapse to summary + Customize (same helpers as mobile). Warm-up summaries list each step on its own line; Custom with no steps shows **Enable warm-up** instead of the full editor. Each non-superset block has its own collapsed **Dropsets** row under that block. Deload Alternate appears only when the exercise is Custom. Setup labels use full words.
