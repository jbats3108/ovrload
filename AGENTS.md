...
- Seeded login (from `database/seeders/UserSeeder.php`): `admin1@test.com` / `password` (admin) and `user1@test.com` / `password` (regular user). `user2@test.com` / `password` has **no default exercise profile**, no demo routines, and one unused custom profile (**Deletable Test**) for Preferences / create-routine testing. Registration is invite-only: leave `REGISTRATION_INVITE` empty locally (no public signup). Admins create one-time links under **Admin → Invites** (email via Resend in prod; Copy / Resend). Optional master secret for bootstrap: `php artisan registration:invite-secret --write` (production only).
...
- Shared exercise catalog: default `database/data/exercises.json` (174 curated commercial-gym lifts from the original short list + selective free-exercise-db picks). Loaded by `ExerciseSeeder` via `ExerciseCatalogImporter` (soft-deletes shared lifts missing from the file). Edit the JSON and re-seed (or call the importer) to refresh local/shared catalog data.
