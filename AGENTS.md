...
- Coverage (Sail / PCOV): with Sail up, `npm run sail:coverage` or `composer test:coverage` writes HTML/Clover/Crap4j under `coverage/` (open `coverage/index.html`). `composer test:coverage:check` / `npm run sail:coverage -- --check` enforces an advisory **50%** floor. CI prints coverage, uploads the HTML artifact, and runs `composer test:coverage:threshold` with `continue-on-error` (same 50% floor; not merge-blocking yet).
