...
- Seeded login (from `database/seeders/UserSeeder.php`): `admin1@test.com` / `password` (admin) and `user1@test.com` / `password` (regular user). `user2@test.com` / `password` has **no default exercise profile**, no demo routines, and one unused custom profile (**Deletable Test**) for Preferences / create-routine testing. Registration is invite-only: leave `REGISTRATION_INVITE` empty locally (no public signup). Admins create one-time links under **Admin → Invites** (email via Resend in prod; Copy / Resend). Optional master bootstrap secret: set `REGISTRATION_INVITE` in prod `.env` manually if needed.
...
- Shared exercise catalog: default `database/data/exercises.json` (174 curated commercial-gym lifts from the original short list + selective free-exercise-db picks). Loaded by `ExerciseSeeder` on re-seed (upsert / restore soft-deleted shared rows; does not prune extras). Edit the JSON and re-seed to refresh local/shared catalog data.
...
- Coverage (Sail / PCOV): with Sail up, `npm run sail:coverage` or `composer test:coverage` writes HTML/Clover/Crap4j under `coverage/` (open `coverage/index.html` / `coverage/dashboard.html` for CRAP). `composer test:coverage:check` / `npm run sail:coverage -- --check` enforces an advisory **90%** floor. CI prints coverage, uploads the HTML artifact, and runs `composer test:coverage:threshold` with `continue-on-error` (same 90% floor; not merge-blocking yet).
- Mutation testing (Sail / Infection): `npm run sail:infection` / `composer test:infection` mutates `Support` + `Policies` + domain `Services` (`infection.json5`); HTML/log under `coverage/infection*`. Covered MSI baseline **79.53%** (2026-09-16; 1831 mutants). No MSI CI gate yet.
- Backend quality: `composer quality-fix` (Rector→Pint×2) / `composer quality` (fix then PHPStan) / `composer analyse` / `composer refactor` / `composer refactor:dry`. CI applies fixes (Rector→Pint×2, ESLint→Prettier) and auto-commits; PHPStan, `vue-tsc`, and PHP coverage threshold are advisory (`continue-on-error`). Larastan level 7 with `parseModelCastsMethod: true`. Rector skips coding-style rules so Pint owns formatting.
...
- **After any local Playwright run that refreshes the DB** (`migrate:fresh` / `E2eSeeder`, including when the agent runs e2e): restore the full local catalog with `./vendor/bin/sail artisan migrate:fresh --seed --force` before handing back. E2e uses a slim exercise set; leaving it in place breaks normal local browsing until re-seeded.
