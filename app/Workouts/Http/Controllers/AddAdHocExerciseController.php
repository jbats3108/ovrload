...
use App\Workouts\Data\Player\StoreAdHocExerciseData;
use App\Workouts\Exceptions\WorkoutServiceException;
use App\Workouts\Models\Workout;
use App\Workouts\Services\WorkoutService;
use Illuminate\Http\RedirectResponse;

class AddAdHocExerciseController extends Controller
{
    public function __invoke(
        Workout $workout,
        StoreAdHocExerciseData $data,
        WorkoutService $workoutService,
    ): RedirectResponse {
        try {
            $workoutService->addAdHocExercise($workout, $data->exerciseId);
